document.addEventListener("DOMContentLoaded", async () => {
    const books = await getBooks('http://localhost:3002/api/book/books');
    if (books) {
...
async function handleDeleteBook(isbn) {
    try {
        const response = await fetch(`http://localhost:3002/api/books/${isbn}`, { method: 'DELETE' });
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
        }
...
async function handleUpdateBook(isbn) {
    try {
        const url = `http://localhost:3002/api/books?isbn=${isbn}`;
...
async function handleLoadBookSummary() {
    try {
        alert("inside hadke book summary23")
        const url = `http://localhost:3002/api/books/summary`
...
async function handleShowBookDetails(isbn) {
    try {
        const url = `http://localhost:3002/api/books/${isbn}`;
        const response = await fetch(url);
...
async function handleLoadListValues(filter) {
    ...
    const books = await getBooks('http://localhost:3002/api/books');
